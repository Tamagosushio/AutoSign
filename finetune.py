...
from utils.utils import generate_autoregressive,decode_labels
from utils.metrics import wer_list
...
def load_checkpoint_for_resume(checkpoint_path, model, optimizer, device):
    ...
    best_wer = checkpoint.get('val_wer', float('inf'))
    
    print(f"Resumed from epoch {start_epoch}, WER: {best_wer:.4f}")
    return start_epoch, best_wer
...
def evaluate_model_with_wer_autoregressive(model, dataloader, device, vocab_info, work_dir, epoch):
    ...
    Returns:
        Tuple[float, float]: Average loss and WER score
    """
            ...
            for batch_idx, batch in enumerate(tqdm.tqdm(dataloader, desc="Autoregressive Eval", ncols=100)):
                ...
                for i in range(pose_values.size(0)):
                    ...
                    generated_ids = generate_autoregressive(
                        ...
                        max_length=20, temperature=0.7, repetition_penalty=1.3
                    )
                    ...
                    pred_file.write(f"Sample {sample_count}: {file_names[i]}\n")
                    pred_file.write(f"GT:   {ground_truth}\n")
                    pred_file.write(f"Pred: {clean_prediction}\n")
                    pred_file.write(f"Generated IDs: {generated_sequence.tolist()}\n")
...
def enhanced_training_pipeline_with_wer(mode, resume_checkpoint=None):
    """
    Training pipeline with WER metrics, best model saving, and early stopping

    This function sets up the training and evaluation process, including data loading, model initialization,
    training loop, and evaluation with WER metrics. It saves model checkpoints and logs predictions.
    
    Returns:
        Dict: Training results including losses, WER scores, and the trained model.
    ...
    work_dir = f"./training_outputs/{mode}"
    ...
    if resume_checkpoint:
        start_epoch, best_wer = load_checkpoint_for_resume(
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        save_checkpoint(model, optimizer, epoch + 1, avg_train_loss, val_loss, val_wer, regular_checkpoint_path)
...
def plot_training_curves_with_wer(results):
    ...
    try:
        ...
        print(f"Individual plots saved: loss_curves.png, wer_curve.png")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--mode', type=str, default='SI', choices=['US', 'SI'])
    parser.add_argument('--resume', type=str, default='models/checkpoint_epoch_37.pt', 
                       help='Path to checkpoint to resume from')

    args = parser.parse_args()

    results = enhanced_training_pipeline_with_wer(args.mode, args.resume)
