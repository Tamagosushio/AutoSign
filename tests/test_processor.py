...
from autosign.processor import AutoSignProcessor
...
def test_tokeniser_with_bos_token():
    tokeniser = AutoSignProcessor(config=AutoSignConfig(), add_bos_token=True)
...
def test_tokeniser_with_eos_token():
    tokeniser = AutoSignProcessor(config=AutoSignConfig(), add_eos_token=True)
    tokeniser_output = tokeniser(texts=["This is a sentence", "That is not a sentence, sorry"])
...
def test_tokeniser_with_eos_and_bos_tokens():
    tokeniser = AutoSignProcessor(config=AutoSignConfig(), add_bos_token=True, add_eos_token=True)
    tokeniser_output = tokeniser(texts=["This is a sentence", "That is not a sentence, sorry"])
...
def test_image_processor():
    ...
    processor = AutoSignProcessor(config=config)
